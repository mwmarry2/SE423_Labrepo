...
// Count variables
uint16_t stateg=0;     //open a global 16 bit integer variable for the state of the buttons
uint32_t numTimer0calls = 0;
uint32_t numSWIcalls = 0;
extern uint32_t numRXA;
uint16_t UARTPrint = 0;
uint32_t numTimer2calls = 0;
uint16_t LEDvalue = 0;   //open a global 16 bit integer variable for the state the LEDs should be in
...
//    init_serialSCID(&SerialD,115200);
    ...
    {
        if (UARTPrint == 1 ) {
			serial_printf(&SerialA,"numTimer2calls:%ld Num SerialRX: %ld state:%d\r\n",numTimer2calls,numRXA,stateg);    //prints the number of timer2 calls, along with the state that the buttons are in as an integer from 0 to 15
...
__interrupt void cpu_timer2_isr(void)
{
    ...
    stateg=ReadSwitches();   //defines the global variable stateg to be the state of the buttons as defined by the function ReadSwitches
    SetLEDsOnOff(stateg);       //sets the LEDs on according to what the stateg variable is, so pushing button 1 turns on LED1, pushing button 2 turns on LED2, etc.
    numTimer2calls++;   //increments the number of timer calls
    UARTPrint = 1;
